...
annotable <- getBM(attributes = c('ensembl_gene_id', 'mgi_symbol'), mart = mart)
annotable <- annotable[!duplicated(annotable[,c('ensembl_gene_id')]),]
annotable_df <- data.frame(annotable[, 2])
rownames(annotable_df) <- annotable$ensembl_gene_id
...
for(j in 1:length(tissue_types)) {
  ...
  for (i in 1:length(target_samples)) {
    ...
    project_output_dir <- paste0(data_dir, "/", project.name, "_gen_202303/", sample)
  ...
  # Add gene symbols and reformat table
  rownames(combined) <- combined$gene_id
  combined <-  merge(combined, annotable_df, by=0)
  names(combined)[names(combined) == 'annotable...2.'] <- 'gene_name'
  combined <- combined[,c(1,ncol(combined),3:(ncol(combined)-1))]
  names(combined)[names(combined) == 'Row.names'] <- 'gene_id'
